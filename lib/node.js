...
class Node {
  ...
  createPublisher(messageType, topic) {
    ...
    let pubHandle = rclnodejs.createPublisher(this._handle);
    let publisher = new Publisher(pubHandle, null, 'message nanme', 'topic name');
  ...
  destoryNode() {
    ...
    this._publishers.forEach((publisher) => {
      // Destroy publisher using rcl
      rclnodejs.destoryEntity('publisher', publisher.handle);
    });
    this._publishers = undefined;
  }
